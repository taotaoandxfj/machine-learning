...
# 定义一个变量用于计算滑动平均,这个变量的初始值设为0.注意这里变量是手动指定了变量的
...
with tf.Session() as sess:
    ...
    # 通过ema.average(v1)来获得滑动平均之后变量的取值.在初始化之后变量v1的值和v1的滑动平均都为0
    print(sess.run([v1, ema.average(v1)]))  # [1.0, 1.0]

    # 更新变量v1的值到5
    sess.run(tf.assign(v1, 5))
    # 更新v1的滑动平均值.衰减率min{0.99,(1+step)/(10+step)=0.999}=0.99
    # 所以v1的滑动平均会被更新为0.99*4.5+0.01*10=4.555
    sess.run(maintain_average_op)
    print(sess.run([v1, ema.average(v1)]))  # [5.0, 4.5999999]

    # 再次更新滑动平均值,得到的新滑动平均值为0.99*4.555+0.01*10=4.60945
    sess.run(maintain_average_op)
    print(sess.run([v1, ema.average(v1)]))  # [5.0, 4.96]
