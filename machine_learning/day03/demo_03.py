...
'''
    设置回归的正确值为两个输入的和加上一个随机量.之所以要加上一个随机量是为了ji
    加入不可预测的噪音,否则不同损失函数的意义就不大了,因为不同损失函数都会在能完全预测正确的
    时候最低,一般来说噪音为了一个均值为0的小量,所以这里的噪音设置为-0.05~0.05的随机值
...
with tf.Session() as sess:
    ...
    for i in range(SETPS):
        ...
        sess.run(trian_step, feed_dict={x: X[start:end], y_: Y[start:end]})
