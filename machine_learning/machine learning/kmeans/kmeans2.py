...
'''
    ...
    tf.slice():具体可以参考https://blog.csdn.net/chenxieyy/article/details/53031943
    对这儿来说就是从vectors的(0,0)处开始取.并且取前k个,这样就达到了随机取四个聚类中心的目的
    或者本目录中的slice函数
...
'''
    ...
    tf.equal()函数表示相等就返回true 否则返回false 但是注意如果传入的是两个列表的话 那么必须两个列表的长度是相等的 如果是单个数字是可以的
    tf.gather():表示用第二个参数中所表示的索引从 第一个参数中取出相应的元素组成新的列表 可参考https://blog.csdn.net/guotong1988/article/details/53172882
    tf.reduce_mean():求平均值,reduction_indices用于维度的限定
    tf.concat()链接函数.0表示纵向链接
    tf.reshape(?,[1,-1]):[1,-1]这个不是很好解释要具体理解可以去看官网的tf.reshape()函数,这儿的目的是为了将tf.where()函数过后的多行转换为1行
    ,从而方便与vectors进行gather函数选取.对于这个的应用后面我的k-means评价也用到了.
'''
...
# 升维方便做减法
for i in range(k):
    expand_data.append(tf.expand_dims(data[i], 0))
...
reduce_sum = []
for i in range(k):
    reduce_sum.append(tf.reduce_sum(tf.reduce_sum(tf.square(tf.subtract(expand_data0, expand_x)), 2)))
reduce_sum1 = tf.reduce_sum(tf.reduce_sum(tf.square(tf.subtract(expand_data[0], expand_x)), 2))
reduce_sum2 = tf.reduce_sum(tf.reduce_sum(tf.square(tf.subtract(tf.reduce_min(reduce_sum[1:]), expand_x)), 2))
